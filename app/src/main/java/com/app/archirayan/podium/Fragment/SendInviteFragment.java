 ...
 */
public class SendInviteFragment extends Fragment {
...
*/
        ...
        emailEdt.addTextChangedListener(new TextWatcher() {
            ...
            public void afterTextChanged(Editable s) {
                if (s.length() > 0) {
                    submitBtn.setBackgroundColor(ContextCompat.getColor(getActivity(), R.color.darkgreen));
                } else {
                    submitBtn.setBackgroundColor(ContextCompat.getColor(getActivity(), R.color.lightgreen));
                }
            }
        });
...
//    http://181.224.157.105/~hirepeop/host1/podium/api/send_invite.php?phone_no=917990028079&name=nik&bussiness_id=27
                try {
                    data = utils.getResponseofGet(Constant.BASE_URL + "send_invite.php?email=" + email + "&name=" + URLEncoder.encode(name, "UTF-8") + "&bussiness_id=" + Utils.ReadSharePrefrence(getActivity(), Constant.UsersBusinessId));
...
//                http://181.224.157.105/~hirepeop/host1/podium/api/send_invite.php?phone_no=+918866793181&name=abcd&buisness=genttaltoyota
                try {
                    data = utils.getResponseofGet(Constant.BASE_URL + "send_invite.php?phone_no=" + email + "&name=" + URLEncoder.encode(name, "UTF-8") + "&bussiness_id=" + Utils.ReadSharePrefrence(getActivity(), Constant.UsersBusinessId));
                } catch (UnsupportedEncodingException e) {
                    e.printStackTrace();
                }
            }
